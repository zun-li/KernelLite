                                         ...
                                         int batch, int size, float eps)
{
    ...
    // float4 要求 16 字节对齐。in / wei 由 cudaMalloc 返回（256B 对齐），
    // 因此只要 size 是 4 的倍数，每行起始地址 bid*size 也就按 16B 对齐。
    // 不满足时把 pack_num 置 0，整行交给下面的标量尾部循环，避免非对齐访问。
    const bool vec_ok =
        (size % pack_size == 0) &&
        ((reinterpret_cast<uintptr_t>(block_in) & 0xF) == 0) &&
        ((reinterpret_cast<uintptr_t>(wei) & 0xF) == 0);
    const int pack_num = vec_ok ? size / pack_size : 0;
...
int main()
{
    ...
    for (int i = 0; i < warpup; i++) {
        row_rmsnorm_f32_dim_simd<<<grid, block>>>
